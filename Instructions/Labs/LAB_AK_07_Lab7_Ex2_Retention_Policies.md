...
1. ブラウザーでは前のタスクで使用したMicrosoft Purviewポータルが開いたままになっており、「アイテム保持ポリシー」画面が表示されているはずです。

3. [アイテム保持ポリシー]画面で、 **[テスト ユーザーの電子メール保持]** の横にあるチェック ボックスをオンにし、メニュー バーで **[ポリシーを無効化]** を選択します。

   ポリシーが無効になると、ポリシーが無効であることを示すメッセージがページの上部に一時的に表示されます。「失敗」と表示された場合も先に進めて構いません(多くの場合「失敗」と表示されても無効化できています)。このタスクの残りの手順に進み、Adatum の公式の組織全体の電子メール保持ポリシーを作成できるようになります。
