...
**※  Holly Dickson(holly@WWLxXXXXXX.onmicrosoft.com) の多要素認証が有効な場合はスキップしてください。**
...
4. 新しい ブラウザー タブに [ユーザーごとの多要素認証]ウィンドウが表示されます。検索欄に、holly と入力して検索します。 Holly Dickson(holly@WWLxXXXXXX.onmicrosoft.com)のチェック ボックスを **オン** にし、上にある **[MFAを有効にする]** を選択します。

   ![](./media/lab6-5-1.png)

5. 表示される[多要素認証を有効にする] ダイアログ ボックスで、 **[有効にする]** ボタンを選択します。

6. ここで、Holly として Microsoft 365 からサインアウトし、ブラウザー セッションを閉じて (キャッシュをクリアするため)、新しいセッションを開いて、MFA を使用して Holly として再度ログインする必要があります。ユーザー アカウントで MFA を有効にした後、初めてサインインし直すと、電話番号や認証オプションなど、MFA に必要な認証情報の入力を求められます。その後、認証プロセスが機能することを検証するための確認コードがテキストメッセージで送信されます。このタスクの残りの部分では、これらの手順を実行します。まずサインアウトし、ブラウザーを閉じます。
...
13. 表示される[別の方法を選択します]ダイアログ ボックスで、 **[電話]** を選択します。
