...
30. [ポリシーの名前を設定する]ページで、[名前] フィールドに **「PII ポリシー」** と入力し、この秘密度ラベル ポリシーに関する次の説明を入力 (またはコピーして貼り付け) します。**このポリシーは情報を暗号化します。**  
...
12. Wordの添付ファイルがあることを確認します。（「安全な添付ファイル」機能のWord添付ファイルスキャンで少々待つ可能性があります）

13. 添付ファイルの右側に表示されている[∨]をクリックし、 次いで [ ↓ ]　(ダウンロード) をクリックします。

14. 画面の右上に[ダウンロード]ページが表示されますので、[Open file(ファイルを開く)]をクリックします。

15. [Sign in]ページが表示されたことを確認し、JoniS@WWLxZZZZZ.onmicrosoft.comで サインインします。パスワードは、  **Pa55w.rd1234** を使用します。

16. [Stay signed in to all your apps(すべてのアプリにサインインしたままにする)]ページが 表示されたことを確認し、**[No, sign in to this app only]** をクリックします。 ([OK]ボタンをクリックしないでください。)

    ![image](./media/lab9-3.png)

17. [Hello Joni,welcome to Office]ページが表示されたことを確認し、**[Continue]** ボタンを クリックします。

18. [Activate Office]ページが表示されたことを確認し、 **JoniS@WWLxZZZZZ.onmicrosoft.com** と入力して（入力されていればそのまま）、**[Next]**　ボタンをクリックします。

19. [Accept the license agreement]ページが表示されたことを 確認し、**[Accept]** ボタンをクリックします。

20. [Your privacy option]ページが表示されたことを確認し、右下の **[Close(閉じる)]** ボタンをクリックします。

21. Joniには、読み取り専用のアクセス許可を付与したため、文書の内容が表示されます。

    また、Wordの画面の上部には通知バーが表示されます。

22. 入力などを行おうとすると、編集がロックされていることを確認します(文字を入力しようとすると、画面下部のステータスバーに変更できない旨のメッセージが表示されます)。
