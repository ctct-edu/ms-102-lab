...
12. [ペイロードの構成]ページで、次の情報を入力します。
    ...
    - 前のメッセージをインポートした後、表示されたテキスト(表示されない場合は、「コード」タブをクリックした後、「テキスト」タブをクリックしてみてください)の最後(～お好みのおもちゃを選択してください:  の後ろ)をクリックしてから、テキスト フォームの上部 (動的 タグの右側) にある **[フィッシング詐欺 リンク]** オプションを選択します。表示される[フィッシング URL に名前を付ける] ダイアログ ボックスで、 [名前]フィールドに **「Free25thAnniversaryGift@tailspintoys.com」** と入力し、  **[確認]** を選択します。
...
4. 新しい ブラウザー タブに [ユーザーごとの多要素認証]ウィンドウが表示されます。

5. 表示されるページで、検索欄に holly と入力して検索します。 Holly Dickson(holly@WWLxXXXXXX.onmicrosoft.com)のチェック ボックスを **オン** にし、上の方にある **[MFAを無効にする]** を選択します。

6. 「多要素認証を無効にする」 ダイアログ ボックスで **[無効]** を選択します。
