...
8. ブラウザーでこの**SharePoint 管理センタータブを閉じます。** 次のタスクのためにブラウザで他のタブを開いたままにしておきます。
...
2. **Windows PowerShell**で、コマンド プロンプトに次のコマンドを入力して、PowerShell ギャラリーから Microsoft Graph PowerShell モジュールをインストールし、Enter キーを押します。

   ```
    Install-Module Microsoft.Graph -Scope CurrentUser
   ```

3. 信頼できないリポジトリ (PSGallery) からモジュールをインストールするかどうかを確認するメッセージが表示されます。**A**を入力して( [A] Yes to All を選択し)、Enter キーを押します。
